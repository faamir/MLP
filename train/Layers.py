...
class Stacked(object):
	...
	def __init__(self, input, n_in, n_out, W = None, b= None, activation_fn=None, seed=35):
		self.input =input
		self.n_in = n_in
		self.n_out = n_out
		...
		if W is None:
			...
			W_values = np.random.uniform(low = -np.sqrt(6./(self.n_in+self.n_out)), high = np.sqrt(6./(self.n_in+self.n_out)),
										 size = (self.n_in, self.n_out))
			
			if activation_fn == T.nnet.sigmoid:
				W_values *= 4
				
			W = theano.shared(name ="Weight",value = W_values, borrow=True)
		...
		l_output = T.dot(self.input, self.W) + self.b
