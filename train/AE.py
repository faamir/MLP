...
enc_layer_2 = Stacked(dec_layer_1.output, 64, nn_input_dim, activation_fn=act_f)
...
#loss = T.nnet.categorical_crossentropy(enc_layer_2.output, x).mean()
loss = 0.5 * T.mean((x - enc_layer_2.output)**2)
prediction = T.argmax(enc_layer_2.output, axis=1)
forward_prop = theano.function([x], enc_layer_2.output, allow_input_downcast=True)
forward_prop2 = theano.function([x], dec_layer_1.output, allow_input_downcast=True)


calculate_loss = theano.function([x], loss, allow_input_downcast=True)
